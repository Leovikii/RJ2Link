...
- **論壇資訊預覽**：在南+點擊 RJ 號，即可查看封面、標題、聲優、標籤、發售日、銷量和快捷入口。
- **跨站資源搜尋**：在 DLsite 開啟懸浮控制項，即可查找對應的南+帖子和 ASMR ONE 試聽頁面。
- **行動端友好**：彈窗始終保持在可見區域內，南+論壇與 DLsite 搜尋結果的長標題不會再觸發頁面或列表橫向捲動。
- **更快的首次預覽**：瀏覽器和網絡空閒時，可提前載入附近作品的輕量資訊。
- **夜間模式與多語言**：提供南+夜間主題，並自動支援簡體中文、繁體中文和英文。
