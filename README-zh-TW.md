...
- **資訊預覽卡片**：在論壇點擊 RJ 號，即可透過不會溢出視窗、內部不滾動的單頁卡片預覽 DLsite 作品元數據（含封面、標題、聲優、差異化分類標籤、僅日期的發售資訊及銷量等）；行動端使用貼合實際可視區域的底部上升彈窗，相容南+缺少 viewport 宣告的舊式頁面且不會改變宿主頁面縮放或橫向寬度，直向高度不超過半屏，緊湊橫向則按可用高度自適應。頁面捲動時彈窗會以對應的下滑動畫收回，行動版論壇的長標題也會限制在原有卡片區域內，避免觸發橫向捲動或覆蓋相鄰資訊。
- **DLsite 全局懸浮艙**：自動注入 DLsite 作品詳情頁，懸停或點擊即可查看 ASMR ONE 與南+檢索結果；右下角控制項使用緊湊的資料來源狀態標籤，懸停時原位淡入，並支援拖曳、分別記憶桌面端與行動端位置，彈窗會貼近控制項並保持在視窗內，多筆結果清單可獨立捲動。南+搜尋會跨分頁排隊以遵守論壇冷卻時間；各資料來源旁的小型重試控制項只重跑對應的失敗或空結果，只有可處理的網絡故障才顯示隱私脫敏診斷資訊。
