...
- **資訊預覽卡片**：在論壇點擊 RJ 號，即可透過不會溢出視窗、內部不滾動的單頁卡片預覽 DLsite 作品元數據（含封面、標題、聲優、差異化分類標籤、僅日期的發售資訊及銷量等）；行動端使用貼合實際可視區域的底部上升彈窗，相容南+缺少 viewport 宣告的舊式頁面且不會改變宿主頁面縮放或橫向寬度，直向高度不超過半屏，緊湊橫向則按可用高度自適應。
