...
- **資訊預覽卡片**：在論壇點擊 RJ 號，即可透過不會溢出視窗的卡片預覽 DLsite 作品元數據（含封面、標題、聲優、差異化分類標籤、僅日期的發售資訊及銷量等）。
- **DLsite 全局懸浮艙**：自動注入 DLsite 作品詳情頁，懸停或點擊即可查看 ASMR ONE 與南+檢索結果；右下角控制項支援拖曳，分別記憶桌面端與行動端位置，彈窗會貼近控制項並保持在視窗內。錯誤和空結果會明確顯示，並提供獨立重試及隱私脫敏診斷資訊。
