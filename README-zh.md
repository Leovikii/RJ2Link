...
- **信息预览卡片**：在论坛点击 RJ 号，即可通过不会溢出视口、内部不滚动的单页卡片预览 DLsite 作品元数据（含封面、标题、声优、差异化分类标签、仅日期的发售信息及销量等）；移动端使用贴合实际可视区域的底部上升弹窗，兼容南+缺少 viewport 声明的旧式页面且不会改变宿主页面缩放或横向宽度，竖屏高度不超过半屏，紧凑横屏则按可用高度自适应。
